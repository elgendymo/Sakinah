...
export default function Phase2Page() {
  ...
  const completedQuestions = Object.keys(state.responses).filter(
    questionId => phase2Questions.some(q => q.questionId === questionId)
  ).length;

  // Progress calculation: 50% base (Phase 1 completed) + up to 25% for Phase 2 completion
  const currentProgress = 50 + (completedQuestions / phase2Questions.length) * 25;
    ...
    >
      ...
      >
        ...
        <motion.div
          ...
          variants={containerVariants}
        >
          {phase2Questions.map((question, index) => (
            <motion.div
              key={question.id}
              variants={{
                initial: { opacity: 0, y: 20 },
        ...
        >
          <NavigationButtons
            ...
            nextLabel={`Continue to Reflection (${completedQuestions}/${phase2Questions.length})`}
        ...
        >
          <p>
            Completed: {completedQuestions} of {phase2Questions.length} questions
